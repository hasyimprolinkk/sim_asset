    ...
    <section class="content-header">
      <h1>
        Asset
        <small>Asset Admin</small>
      </h1>
      <ol class="breadcrumb">
      	<li class=""><i class="fa fa-asset"></i> Asset</li>
        <li class="active"><i class="fa fa-asset-plus"></i> Add Asset</li>
      </ol>
    </section>
			            ...
			            <?= form_open_multipart(); ?>
			              ...
			              <div class="box-body">
							...
							<div class="form-group <?= form_error('id_jenis') ? 'has-error' : null; ?>">
			                  ...
			                  <select name="id_jenis" class="form-control" id="id_jenis">
								...
								<?php foreach($jenis as $j) : ?>
									<?php if(set_value('id_jenis') == $j['id_jenis']) : ?>
							...
							<div class="form-group <?= form_error('id_kategori') ? 'has-error' : null; ?>">
			                  ...
			                  <select name="id_kategori" class="form-control" id="id_kategori">
								...
								<?php foreach($kategori as $j) : ?>
									<?php if(set_value('id_kategori') == $j['id_kategori']) : ?>
			                ...
			                <div class="form-group <?= form_error('kondisi') ? 'has-error' : null; ?>">
			                  ...
			                  <input type="text" name="kondisi" class="form-control" id="kondisi" value="<?= set_value('kondisi'); ?>">
			                  <?= form_error('kondisi'); ?>
			                </div>

			                <div class="form-group <?= form_error('nopol') ? 'has-error' : null; ?>">
			                  <label for="nopol">Nopol*</label>
			                  <input type="text" name="nopol" class="form-control" id="nopol" value="<?= set_value('nopol'); ?>">
			                  <?= form_error('nopol'); ?>
			                </div>

							<div class="form-group <?= form_error('km') ? 'has-error' : null; ?>">
			                  <label for="km">KM Perjalanan*</label>
			                  <input type="number" name="km" class="form-control" id="km" value="<?= set_value('km'); ?>">
			                  <?= form_error('km'); ?>
			                </div>
